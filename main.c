...
#pragma region VULKAN BASE
...
#endif
#if !defined(_WIN32)
    // to lower latency on kde, put in the launch script at
    // ~/.config/plasma-workspace/env/kwin_env.sh < export KWIN_DRM_DISABLE_TRIPLE_BUFFERING=1
    // so that it takes effect when kwin launches
    // it actually shaves a frame of latency from 4 to 3 frames it seems
#endif
    ...
    // must match the GBM/KMS scanout format in drm_present.c
    color_format = VK_FORMAT_B8G8R8A8_UNORM;
    // dummy, not needed for drm/kms path
...
#pragma region PIPELINES
    ...
    VkPipelineRenderingCreateInfo pr = {
        ...
        .pColorAttachmentFormats = (VkFormat[]){ color_format, VK_FORMAT_R32_UINT },
...
#pragma region PICK OBJECT, PICK RECT, SELECT WORLD POSITION
        ...
        if (g_drag_active) {
            ...
            if (max_sx > min_sx && max_sy > min_sy) {
                rect_valid = 1;
                rect_x0 = (float)min_sx / (float)targets.extent.width;
                rect_y0 = (float)min_sy / (float)targets.extent.height;
                rect_x1 = (float)max_sx / (float)targets.extent.width;
                rect_y1 = (float)max_sy / (float)targets.extent.height;
            }
        ...
        if (buttons[MOUSE_LEFT]) {
            ...
            // clamp to screen
            uint32_t w = targets.extent.width;
            uint32_t h = targets.extent.height;
        ...
        if (buttons[MOUSE_RIGHT]) {
            ...
            if (x >= targets.extent.width || y >= targets.extent.height) return 0;
            VkCommandBuffer cmd = begin_single_use_cmd(machine.device, command_pool);
            ...
            vkCmdCopyImageToBuffer(
                cmd,
                targets.depth_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            ...
            if (depth > 0.0f) {
                float ndc_x =  2.0f * ((x + 0.5f) / (float)targets.extent.width)  - 1.0f;
                float ndc_y =  1.0f - 2.0f * ((y + 0.5f) / (float)targets.extent.height);
        ...
        if (g_want_pick) {
            ...
            g_want_pick = 0;
            // Clamp to viewport
            if (g_mouse_x < targets.extent.width && g_mouse_y < targets.extent.height) {
                ...
                vkCmdCopyImageToBuffer(
                    cmd,
                    targets.pick_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
...
#pragma region upload uniforms
        ...
        /* For DRM, command buffers are per-backbuffer, not per-swapchain image.
           For non-DRM, this is still per swapchain image. */
        if (!command_buffers_recorded[image_index]) {
...
#if DEBUG_APP == 1
            uint32_t q0 = image_index * QUERIES_PER_IMAGE + 0;
            vkCmdResetQueryPool(cmd, targets.query_pool, q0, QUERIES_PER_IMAGE);
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT, targets.query_pool, q0 + Q_BEGIN);
#endif
...
#if DEBUG_APP == 1
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_TRANSFER_BIT, targets.query_pool, q0 + Q_CHUNKS);
#endif
...
#if DEBUG_APP == 1
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, targets.query_pool, q0 + Q_OBJECTS);
#endif
            ...
            VkBufferMemoryBarrier2 prepare_to_scatter_indirect =
                buf_barrier2(ST_CS, AC_SWR, VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT, AC_IND,
            ...
            prep2[0] = buf_barrier2(ST_CS, AC_SWR, ST_CS, AC_SRD, buffer_offset_per_mesh, 0, VK_WHOLE_SIZE);
            prep2[1] = buf_barrier2(ST_CS, AC_SWR, ST_CS, AC_SRD | AC_SWR, buffer_draw_calls, 0, VK_WHOLE_SIZE);
            cmd_barrier2(cmd, NULL, 0, prep2, 2, NULL, 0);

#if DEBUG_APP == 1
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, targets.query_pool, q0 + Q_PREPARE);
#endif
...
#if DEBUG_APP == 1
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT, targets.query_pool, q0 + Q_SCATTER);
#endif

            /* RENDER TARGET TRANSITIONS */
            VkImageMemoryBarrier2 to_depth = img_barrier2(
                VK_PIPELINE_STAGE_2_NONE, 0,
                ST_EFT, AC_DSWR | AC_DSRD,
                VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL,
                targets.depth_image, VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 0, 1);
            cmd_barrier2(cmd, NULL, 0, NULL, 0, &to_depth, 1);

            VkImageMemoryBarrier2 to_color = img_barrier2(
                VK_PIPELINE_STAGE_2_NONE, 0,
            ...
            VkImageMemoryBarrier2 to_pick = img_barrier2(
                VK_PIPELINE_STAGE_2_NONE, 0,
                VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT, AC_CWR,
                VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                targets.pick_image, VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1);
            cmd_barrier2(cmd, NULL, 0, NULL, 0, &to_pick, 1);

#if DEBUG_APP == 1
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT, targets.query_pool, q0 + Q_BUCKETS);
#endif
            ...
            color_atts[1] = (VkRenderingAttachmentInfo){
                .sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
                .imageView = targets.pick_image_view,
                .imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
            ...
            mode = UI_MODE;
            vkCmdPushConstants(cmd, common_pipeline_layout,
                VK_SHADER_STAGE_COMPUTE_BIT | VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                0, sizeof(enum mode), &mode);
...
#if DEBUG_APP == 1
            vkCmdWriteTimestamp2(cmd, VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT, targets.query_pool, q0 + Q_RENDER);
#endif
            ...
            command_buffers_recorded[image_index] = 1;
...
#if DEBUG_APP == 1
        presented_frame_ids[image_index] = frame_id;
        start_time_per_slot[image_index] = frame_start_time;
        frame_id_per_slot[image_index] = frame_id;
...

{
    ...
    VK_CHECK(vkWaitSemaphores(machine.device, &render_done, UINT64_MAX));
...

        {
            ...
            if (last_frame_id != (uint64_t)-1) {
                uint32_t q_first = image_index * QUERIES_PER_IMAGE;
...
{ .sType = VK_STRUCTURE_TYPE_CALIBRATED_TIMESTAMP_INFO_EXT, .timeDomain = VK_TIME_DOMAIN_CLOCK_MONOTONIC_EXT }
...
#endif
                    offset_ns = cpu_now_ns - gpu_now_ns;
                }
                ...
                if (qr == VK_SUCCESS) {
                    ...
                    double ms_end     = (ns[Q_END]     - ns[Q_BLIT])    * 1e-6;
                    double ms_total   = (ns[Q_END]     - ns[Q_BEGIN])   * 1e-6;

                    double cpu_ms_begin = (offset_ns + ns[Q_BEGIN] - pf_ns_start()) * 1e-6;
                    double cpu_ms_end   = (offset_ns + ns[Q_END]   - pf_ns_start()) * 1e-6;

                    printf("[%llu] gpu time %.3fms - %.3fms : chunks=%.3f, objects=%.3f, prepare=%.3f, scatter=%.3f, buckets=%.3f, render=%.3f, blit=%.3f, end=%.3f, [%.3f]\n",
                           (unsigned long long)last_frame_id, cpu_ms_begin, cpu_ms_end,
                           ms_chunks, ms_objects, ms_prepare, ms_scatter, ms_buckets, ms_render, ms_blit, ms_end, ms_total);
                } else {
                    printf("Cannot get query results for %llu: %s\n",
                           (unsigned long long)last_frame_id, vk_result_str(qr));
                }
            }
        }
#endif
